...
$query = "SELECT v.fecha, CONCAT(u.paterno,' ',u.materno,' ',u.nombres) as uNombres, CONCAT(c.paterno,' ',c.materno,' ',c.nombres)as Nombres, v.total, SUM(d.cantidad) as suma, p.tipo_pago FROM usuario u, cliente c, venta v ,detalle d, moto m, pagos p WHERE (v.idventa = d.nventa AND c.idcliente = v.idcliente) AND (u.idusuario = v.idusuario) AND (v.idventa = '$id' AND v.estado_venta=1 ) AND d.codmoto = m.codmoto AND p.idpago = v.idpago" ;
...
$pdf->Cell(15,5,'Monto adelantado Bs.'.$fila['4']*300,0,1,'L');
$pdf->Ln(5);
